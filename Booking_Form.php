...
if(isset($savedata))
{
  ...
  $rtimefinal = str_replace(':', '', $rtime);

  $rtimelow=(int)$rtimefinal;
  $low=$rtimelow-30;
  if ($low%100>=31)
  {
    $low=$low-40;
  }
  $rtimehigh=(int)$rtimefinal;
  $high=$rtimehigh+30;
  if($high%100>=31)
  {
    $high=$high+40;
  ...
  {
    $conflict = mysqli_query($con,"select * from service_reservation_details where reservation_date='$rdate' and r_time>=(CAST('$low' AS int)) and r_time<=(CAST('$high' AS int))");

    if(mysqli_num_rows($conflict))
    { 
    $msg= "<h1 style='color:red'>This time is already reserved.</h1>";
    }
    else
    {
    $sql="insert into service_reservation_details(id,name,email,phone,address,country,service_type,reservation_date,reservation_time,r_time) 
    values('$i','$name','$eid','$phone','$address','$country','$type','$rdate','$rtimefinal',CAST('$rtimefinal' AS int))";
